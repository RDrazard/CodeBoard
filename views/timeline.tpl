...
%include shared/form.tpl
	...
	%for tweet in postlist:
		<p><img src="{{ get_url('static', path='assets/img/avatar.png') }}" /> <strong><a href="/{{tweet['uid']}}">{{tweet['uid']}}</a></strong> {{tweet['content']}}<span><a href="/{{tweet['uid']}}/statuses/{{tweet['_id']}}">permalink</a></span></p>
	%end
	</div>
</div>
