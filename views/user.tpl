...
<div id="main">
	...
	%if postlist:
  	%for tweet in postlist:
  		<p><img src="{{ get_url('static', path='assets/img/avatar.png') }}" /> <strong><a href="/{{tweet['uid']}}">{{tweet['uid']}}</a></strong> {{tweet['content']}}<span><a href="/{{username}}/statuses/{{tweet['_id']}}">permalink</a></span></p>
  	%end
  %else:
